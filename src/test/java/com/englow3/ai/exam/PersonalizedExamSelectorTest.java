...
class PersonalizedExamSelectorTest {
    ...
    void excludesItemsOutsideRequestedDifficultyWindow() {
        ...
        List<Candidate> selected = selector.select(List.of(easy, medium, hard), PersonalizedExamDtos.Skill.READING, 10,
                new BigDecimal("0.4"), new BigDecimal("0.6"), seed);
    ...
    void mixedBlueprintUsesDeterministicListeningReadingSplit() {
        ...
        assertThat(
                selector.select(candidates, PersonalizedExamDtos.Skill.MIXED, 5, BigDecimal.ZERO, BigDecimal.ONE, seed))
                        .extracting(Candidate::itemId)
