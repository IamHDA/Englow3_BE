...
class AiJobEventPublisherTest {
    ...
    void terminalPayloadContainsOnlyOperationalIdentifiers() {
        var input = JsonNodeFactory.instance.objectNode().put("learnerText", "private transcript").put("systemPrompt",
                "private prompt");
        AiJob job = AiJob.pending(UUID.randomUUID(), AiCapability.SPEAKING, "SPEAKING_ASSESSMENT", "SPEAKING_SESSION",
                UUID.randomUUID(), "ai-service", "speech-model", "2", input, "request-1", "trace-1");
