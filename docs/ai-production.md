...
- `AI_ENABLED=true`, `AI_API_KEY`, `AI_BASE_URL`, `AI_DEFAULT_MODEL`.
- `S3_ENDPOINT`, `S3_ACCESS_KEY`, `S3_SECRET_KEY`, `S3_BUCKET` for a private bucket.
- For speaking: `SPEECH_ENABLED=true`, `AZURE_SPEECH_BASE_URL`, `AZURE_SPEECH_API_KEY`.
...
1. Run `mvn test`. `FlywayMigrationTest` applies every migration to PostgreSQL when Docker is available and skips only on developer machines without Docker.
2. Run `mvn formatter:validate` and `mvn package`.
3. Deploy with AI disabled first and verify `/actuator/health`, `/actuator/health/liveness`, and `/actuator/health/readiness`.
4. Configure prompt/model policies, canary one capability, then enable the remaining capabilities.
5. Alert on `englow3.ai.requests{outcome="failure"}`, latency, queue depth, recent failures and provider quota errors.
6. Verify the object-storage lifecycle is at least as strict as `SPEECH_AUDIO_RETENTION`; the application also deletes expired recordings.
