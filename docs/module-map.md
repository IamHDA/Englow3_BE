...
- **Exam authoring endpoints.** Content is seeded by SQL in this phase. Admin can create a paper shell and publish it, but cannot fill it through the API - sections, parts, question sets and questions have no write path. **Authorization is decided but not built** (see `shared + config` and *Admin exam management*): the design calls for a `Role` enum in `user`, the Supabase token hook carrying `userId` + `role`, a `JwtAuthenticationConverter` mapping the claim to an authority, and `@PreAuthorize` on `/api/admin/**` - none of it exists in code today. There is no `@PreAuthorize` anywhere, claims are never mapped to authorities, `CurrentUser` exposes only `authProviderId` and `email`, and `users.role` is a `String` no code reads; `@EnableMethodSecurity` and the `AccessDeniedException` handler are pre-wired hooks waiting for it. Still open beyond writing that code: granting the hook read access to `englow3.users`, and what an admin does when their token was issued before the role was granted (today: sign out and back in).
